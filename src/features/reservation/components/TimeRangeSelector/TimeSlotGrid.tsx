...
const TimeSlotGrid = memo(function TimeSlotGrid({
  ...
  if (loading) {
    return (
      <div className="h-[116px] border border-pronto-gray-200 bg-white rounded-sm overflow-hidden relative">
        <div className="w-full p-2 flex items-center">
          <div className="animate-pulse flex space-x-2 w-full overflow-hidden">
            {/* 시간 스켈레톤 - 더 부드러운 로딩 애니메이션 */}
            {Array.from({ length: 12 }).map((_, index) => (
              <div 
                key={index} 
                className="h-10 w-10 bg-pronto-gray-100 rounded flex-shrink-0"
                style={{
                  opacity: 1 - (index * 0.05), // 점점 투명해지는 효과
        ...
        <div className="absolute inset-0 bg-white/30 backdrop-blur-[1px] flex items-center justify-center">
          <div className="w-6 h-6 border-2 border-pronto-primary border-t-transparent rounded-full animate-spin"></div>
        </div>
      </div>
    );
  }
