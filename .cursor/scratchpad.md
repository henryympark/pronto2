...
### 추가 작업 완료
- [x] API에서 cancelled 상태를 제외한 모든 예약 상태(completed, modified 포함) 조회하도록 수정
- [x] React Query 캐시 시간 개선 (오늘 날짜: 1분, 윈도우 포커스 시 재요청)
- [x] HTTP 캐시 시간 단축 (오늘 날짜: 30초)
- [x] 예약 완료 후 캐시 무효화 로직 추가
