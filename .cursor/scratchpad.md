...
### 8. 리뷰 작성 시 적립시간 부여 문제 해결 (신규)
- [x] 문제 분석 완료: 리뷰 작성 페이지에서 적립시간 부여 로직 누락 확인
- [x] 리뷰 작성 완료 후 review-reward Edge Function 호출 로직 추가
- [x] API 라우트 생성: `/api/supabase/functions/review-reward/route.ts`
- [x] 적립시간 부여 성공/실패 처리 및 사용자 피드백 개선
- [x] **문제 원인 파악**: 환경 변수 `SUPABASE_SERVICE_ROLE_KEY` 미설정으로 API 라우트 실패
- [x] **해결 방안 적용**: 리뷰 작성 페이지에서 직접 데이터베이스 업데이트하도록 수정
- [ ] 테스트: 리뷰 작성 후 실제 적립시간 증가 확인

**문제 분석 결과:**
1. **Edge Function은 정상 작동**: 직접 호출 시 적립시간이 40분 → 50분으로 증가 확인
2. **API 라우트 문제**: 환경 변수 `SUPABASE_SERVICE_ROLE_KEY`가 설정되지 않아 Edge Function 호출 실패
3. **RLS 정책 제한**: anon key로는 customers 테이블 접근 불가

**최종 해결 방안:**
- API 라우트를 거치지 않고 리뷰 작성 페이지에서 직접 Supabase 클라이언트로 적립시간 업데이트
- 사용자 인증된 상태에서 실행되므로 RLS 정책 통과 가능
- 코드 간소화 및 의존성 감소

**현재 상태:**
- test@pronto.com 계정: 리뷰 6개 작성, 적립시간 50분 (정상: 60분, 부족: 10분)
- Edge Function 테스트로 10분 추가되어 현재 50분
...
리뷰 작성 시 적립시간 부여 문제 해결 작업은 아직 완료되지 않았습니다. 이 작업을 완료하면 리뷰 작성 후 적립시간이 실제로 부여되는지 확인할 수 있습니다.
...
# 빌드 오류 수정 및 타입 안전성 개선 작업 (2024-12-19)

## Background and Motivation

프로젝트 빌드 테스트 과정에서 여러 타입 오류와 호환성 문제가 발견되었습니다. 주요 문제는 useToast 훅의 사용법 불일치, 타입 불일치, 그리고 Supabase 클라이언트 호출 오류였습니다. 이러한 문제들을 해결하여 프로젝트의 타입 안전성을 향상시키고 빌드 오류를 제거하고자 합니다.

## Key Challenges and Analysis

1. **useToast 훅 호환성 문제**: 여러 파일에서 `const { toast } = useToast()` 방식과 `import { toast }` 방식이 혼재되어 타입 오류 발생
2. **ServiceDetailClient 타입 불일치**: Service 타입을 Studio 타입이 필요한 곳에 전달하여 타입 오류 발생
3. **createClient 함수 호출 오류**: 인자 없이 호출되어 타입 오류 발생
4. **AvailableTimesApiResponse 타입 충돌**: 여러 파일에서 동일한 이름의 다른 타입 정의로 인한 충돌

## High-level Task Breakdown

1. [x] useToast 훅 호환성 개선
   - [x] `src/hooks/use-toast.ts` 파일 수정하여 함수 호출과 객체 접근 방식 모두 지원
   - [x] Object.assign을 사용해 toast 함수에 메서드 추가
   - [x] 성공 기준: 모든 파일에서 toast 사용 시 타입 오류 없음

2. [x] 각 파일의 toast 사용법 통일
   - [x] `src/app/password/reset/page.tsx`: toast.success(), toast.error() 방식으로 변경
   - [x] `src/app/payment/complete/demo/page.tsx`: toast.success() 방식으로 변경
   - [x] `src/app/payment/complete/page.tsx`: toast.success() 방식으로 변경
   - [x] `src/app/signup/page.tsx`: toast 호출 방식 통일
   - [x] 성공 기준: 모든 파일에서 일관된 toast 사용법 적용

3. [x] ServiceDetailClient 타입 불일치 해결
   - [x] Service를 Studio 타입으로 변환하는 로직 추가
   - [x] useMemo를 사용해 Service 데이터를 Studio 형태로 변환
   - [x] 성공 기준: ServiceDetailClient에서 타입 오류 없음

4. [x] createClient 함수 호출 오류 수정
   - [x] `src/domains/booking/hooks/useAvailability.ts`에서 createClient$ 사용
   - [x] 성공 기준: Supabase 클라이언트 호출 시 타입 오류 없음

5. [x] 변경사항 커밋 및 GitHub 업데이트
   - [x] 모든 변경사항을 Git에 커밋
   - [x] GitHub 원격 저장소에 푸시
   - [x] 성공 기준: 모든 변경사항이 GitHub에 반영됨

## Project Status Board

- [x] useToast 훅 호환성 개선 - 완료
- [x] 각 파일의 toast 사용법 통일 - 완료
- [x] ServiceDetailClient 타입 불일치 해결 - 완료
- [x] createClient 함수 호출 오류 수정 - 완료
- [x] 변경사항 커밋 및 GitHub 업데이트 - 완료

## Current Status / Progress Tracking

빌드 오류 수정 및 타입 안전성 개선 작업을 성공적으로 완료했습니다:

1. **타입 안전성 대폭 향상**: useToast 관련 타입 오류들이 모두 해결되어 프로젝트의 타입 안전성이 크게 개선되었습니다.

2. **코드 일관성 확보**: toast 사용법을 통일한 것처럼, 프로젝트 전반에서 일관된 패턴을 사용하면 유지보수성이 크게 향상됩니다.

3. **호환성 유지**: 기존 코드를 최대한 보존하면서 새로운 방식도 지원하도록 개선했습니다.

4. **성능 최적화**: 타입 변환 로직에서 useMemo를 사용한 것처럼, 타입 안전성을 확보하면서도 성능을 고려해야 합니다.
...
1. **타입 호환성 설계의 중요성**: useToast 훅에서 발생한 문제처럼, 라이브러리나 훅을 설계할 때 다양한 사용 패턴을 고려해야 합니다. Object.assign을 사용한 해결책은 함수와 객체의 특성을 모두 활용할 수 있는 좋은 예시입니다.

2. **점진적 마이그레이션 전략**: 기존 코드를 한 번에 모두 변경하는 대신, 호환성을 유지하면서 점진적으로 개선하는 것이 안전합니다.

3. **타입 변환 로직의 활용**: ServiceDetailClient에서 Service를 Studio로 변환한 것처럼, 타입 간 변환 로직을 적절히 활용하면 기존 구조를 크게 변경하지 않고도 타입 안전성을 확보할 수 있습니다.

4. **일관된 코딩 스타일의 중요성**: toast 사용법을 통일한 것처럼, 프로젝트 전반에서 일관된 패턴을 사용하면 유지보수성이 크게 향상됩니다.

5. **빌드 오류 해결의 우선순위**: 타입 오류는 런타임 오류로 이어질 수 있으므로 ESLint 경고보다 우선적으로 해결해야 합니다.

6. **Git 커밋 메시지의 중요성**: 변경사항을 명확히 기록하는 커밋 메시지는 나중에 코드 히스토리를 추적할 때 매우 유용합니다.

7. **성능 최적화 고려**: 타입 변환 로직에서 useMemo를 사용한 것처럼, 타입 안전성을 확보하면서도 성능을 고려해야 합니다. 